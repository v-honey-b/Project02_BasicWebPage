...
public class BoardController extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		case "boarddetail" :		// 세부 글 보기
			...
			BoardVO bvo = bsv.getBoardDetail(bno);			// db에 해당 bno의 값을 검색해서 bno의 객제를 반환
			req.setAttribute("bvo", bvo);				// 반환된 bvo를 boarddetail 페이지로 보냄
			log.info(">>> boarddetail >>> OK");
			destPage = "/board/boarddetail.jsp";
			break;
		case "boardmodify" :		// 수정 페이지로 이동
			req.setAttribute("bvo", bsv.getBoardDetail(Integer.parseInt(req.getParameter("bno"))));
			log.info(">>> boardmodify >>> OK");
			destPage = "/board/boardmodify.jsp";
			break;
		case "boardupdate" :		// 실제 수정이 일어나는 부분
			isUp = bsv.boardupdate(new BoardVO(Integer.parseInt(req.getParameter("bno")), req.getParameter("title"), req.getParameter("content")));
			log.info(">>> boardupdate >>> " + (isUp > 0 ? "OK" : "FAIL"));
			destPage = "boarddetail?bno="+Integer.parseInt(req.getParameter("bno"));
			break;
		case "boardremove" :		// 실제 삭제가 일어나는 부분
			isUp = bsv.boardremove(Integer.parseInt(req.getParameter("bno")));
			log.info(">>> boardremove >>> " + (isUp > 0 ? "OK" : "FAIL"));
			destPage = "boardlist";
			break;
		}
