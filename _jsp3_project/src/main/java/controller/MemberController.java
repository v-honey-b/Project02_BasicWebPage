...
public class MemberController extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		case "membermodify" : 	// 번호를 주고 해당 번호의 값 1개를 검색해서 open 파일에 전달 
			req.setAttribute("mvo", msv.getMemberDetail(req.getParameter("email")));
			destPage = "/member/membermodify.jsp";
			break;
		case "memberupdate" : 	// 실제 수정이 일어나는 위치
			isUp = msv.memberupdate(new MemberVO(req.getParameter("email"), req.getParameter("pwd"), req.getParameter("nick_name")));
			log.info(">>> update : " + (isUp > 0 ? "OK" : "FAIL"));
			destPage = "memberlist";
			break;
		case "memberremove" : 	// 실제 삭제가 일어나는 위치
			isUp = msv.memberremove(req.getParameter("email"));
